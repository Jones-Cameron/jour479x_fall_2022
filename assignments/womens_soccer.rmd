...
NIJT, Pacific, St. Bonaventures, and VCU
...
matches_2022 %>% 
  filter(team == "Maryland Terrapins, Terps") %>% 
  group_by(team) %>% 
  summarise(total = sum(corners))
#31 corners

matches_2022 %>% 
  filter(team == "Temple Owls", "Navy Midshipmen", "Penn Quakers", "Georgetown Hoyas", "George Mason Patriots") %>% 
  group_by(team) %>% 
  summarise(total = sum(corners))
